...
void modificar() {
	...
	for (int j = 0; j <= 99; j++) {
		...
		if (strcmp(estructura[j].ID, b) == 0 && estructura[j].ID[0] != 0) {
			...
			cout << "6. Modificar el genero" << endl;
			cout << "7. Modificar el precio unitario" << endl;
			cout << "Ingrese el numero de la opcion que desee realizar: ";
			op1 = _getch();
			switch (op1) {
			case 49:
...
void eliminar() {
	bool bandera = true;
	/*for (int j = 0; j <= 50; j++) {
		estructura[100].ID[j] = NULL;
	}
	name[100] = '\0';
	for (int j = 0; j < 50; j++) {
		estructura[100].anio[j] = NULL;
	}
	clasi[100] = '\0';
	cara[100] = '\0';
	des[100] = '\0';
	...
	for (int it = 0; it <= 99; it++) {
		if (strcmp(gg, estructura[it].ID) == 0) {
			for (it; it <= 98; it++) {//seguimiento: como el valor 100 no existe aqui se arma un pedo entonces tengo q hacer un if para cuando sea 99 pero tengo hueva asi que me voy a dormir
				if (estructura[it + 1].ID[0] != NULL) {
					for (int p = 0; p < 50; p++) {
...
void vigentes() {
	...
	case 50:
		...
		for (int it = 0; it <= 99; it++) {
			if (strcmp(eve, cambio[it].genchar) == 0) {
	...
	case 51:
		...
		for (int it = 0; it <= 99; it++) {
			if (strcmp(vicks, cambio[it].clasichar) == 0) {
				cout << it + 1 << "-El ID del videojuego es: " << estructura[it].ID << endl;
				cout << "El nombre del videojuego es: " << name[it] << endl;
				cout << "El anio del videojuego es: " << estructura[it].anio << endl;
				cout << "La clasificacion del videojuego es: " << clasi[it] << endl;
